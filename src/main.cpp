...
int main()
{
    ...
    std::cout
        << "[SYSTEM] Node 0x1001 initialized successfully."
        << std::endl;

    // 2. Simulated non-blocking mesh maintenance cycles
    // Heartbeats are generated every 2000 ms.
    // Stale nodes are removed after 6000 ms.
    for (uint32_t current_time_ms = 0;
         current_time_ms <= 10000;
         current_time_ms += 1000)
    {
        local_node.send_heartbeat(
            current_time_ms
        );

        local_node.purge_stale_nodes(
            current_time_ms
        );

        TelemetryLogger::log_node_health(
            local_node,
            current_time_ms
        );
    }
    ...
    tx_packet.header.sender_id =
        0x1001;

    tx_packet.header.receiver_id =
        0x1002;

    tx_packet.header.sequence_num =
        1;

    tx_packet.header.ttl =
        5;
